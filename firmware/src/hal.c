...
uint32_t hal_id() {
    ...
    // based on lowhash32 from https://nullprogram.com/blog/2018/07/31/
    uint32_t id[3] = {0};
...
bool hal_tx_ready() {
	return usart_get_flag(USART1, USART_FLAG_TXE);
}
...
bool hal_rx_ready() {
	return usart_get_flag(USART2, USART_FLAG_RXNE) || usart_get_flag(USART2, USART_FLAG_ORE);
}

uint16_t hal_rx() {
    USART_ISR(USART2) &= ~USART_ISR_ORE;
